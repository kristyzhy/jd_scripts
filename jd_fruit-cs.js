...
*/
const $ = new Env('东东农场');
let cookiesArr = [], cookie = '', jdFruitShareArr = [], isBox = false, notify, notifyb, newShareCodes, allMessage = '', notifyMsg = '';
...
!(async () => {
  ...
  if ($.isNode() && allMessage && $.ctrTemp) {
    ...
    await notifyb.sendNotify(`${$.name}`, `${notifyMsg}`)
  }
})()
    .catch((e) => {
      $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    notifyb = $.isNode() ? require('./sendNotify1.js') : '';
